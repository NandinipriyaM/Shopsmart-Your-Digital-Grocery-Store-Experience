...
app.put('/products/:id', async (req, res) => {
    try {
        const { productname, price, description, image, category, countInStock, rating } = req.body;
...
app.put('/update-cart', async (req, res) => {
    ...
    try {
        const updatedCartItem = await models.AddToCart.findOneAndUpdate(
            { userId, productId },
            { $set: { quantity } },
            { new: true }
        );
        ...
        res.status(200).json({ message: 'Quantity updated', data: updatedCartItem });
    } catch (error) {
